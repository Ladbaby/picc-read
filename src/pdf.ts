...
//   - `execFileNoThrow` → local adaptor (node:child_process).
// =============================================================================
...
/** Directory for extracted PDF page images. */
function getToolResultsDir(): string {
  return join(tmpdir(), "pi-pdf-extracts");
}
...
/** Check whether the `pdftoppm` binary (poppler-utils) is available (cached). */
...
export async function extractPDFPages(
  ...
  try {
    ...
    return {
      ...
      data: {
        type: "parts",
        file: { filePath, originalSize, outputDir, count: imageFiles.length },
      },
    };
  } catch (e: unknown) {
    return { success: false, error: { reason: "unknown", message: errorMessage(e) } };
