...
//   - `Base64ImageSource`/`ImageBlockParam` are Anthropic SDK types; picc-read
//     only needs the string media_type, so those are simplified.
// =============================================================================
 ...
 * Ported from `utils/imageResizer.ts:maybeResizeAndDownsampleImageBuffer`.
 */
...
/** Resizes an image block (base64) if needed. */
export async function maybeResizeAndDownsampleImageBlock(
  ...
  if (imageBlock.source.type !== "base64") {
    return { block: imageBlock as ImageBlockWithDimensions["block"] };
  }
...
/** Create a metadata text block for image dimensions + source path. */
